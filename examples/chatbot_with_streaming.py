...
class ChatBot:
    def __init__(
        ...
        if not api_key:
            raise ValueError("An API key must be provided to use the OpenVoid API.")
        self.client = OpenVoidClient(api_key=api_key)
        self.model = model
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A simple chatbot using the OpenVoid API"
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("OPENVOID_API_KEY"),
        help="OpenVoid API key. Defaults to environment variable OPENVOID_API_KEY",
    )
    parser.add_argument(
        "-m",
        "--model",
